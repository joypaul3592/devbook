export type Locale = "bn" | "en";

export const translations = {
  bn: {
    // Navigation
    nav: {
      learn: "শেখা",
      articles: "লেখা",
      journal: "জার্নাল",
      series: "সিরিজ",
      projects: "প্রজেক্ট",
      about: "পরিচয়",
      search: "সার্চ",
      menuOpen: "মেনু খুলুন",
      menuClose: "মেনু বন্ধ করুন",
      home: "হোম",
    },
    // Footer
    footer: {
      tagline: "একজন বাংলাদেশী ডেভেলপারের ব্যক্তিগত নোটবুক",
      description:
        "ওয়েব ডেভেলপমেন্ট, ডিজাইন সিস্টেম আর প্রতিদিনের কোডিং অভিজ্ঞতা নিয়ে লেখা একটি ব্যক্তিগত নোটবুক — যেখানে শেখা, তৈরি করা আর ভাবনাগুলো যত্ন করে সাজিয়ে রাখা হয়।",
      articles: "লেখা",
      journal: "জার্নাল",
      series: "সিরিজ",
      projects: "প্রজেক্ট",
      about: "পরিচয়",
      uses: "টুলস",
      contact: "যোগাযোগ",
      copyright: "যত্ন সহকারে তৈরি",
      sectionContent: "কনটেন্ট",
      sectionMore: "আরও",
      sectionConnect: "যোগাযোগ",
      contactUs: "যোগাযোগ করুন",
      privacy: "প্রাইভেসি পলিসি",
      terms: "শর্তাবলী",
      sitemap: "সাইটম্যাপ",
      available: "নতুন কাজের জন্য উন্মুক্ত",
      backToTop: "উপরে যান",
    },
    // Homepage
    home: {
      eyebrow: "ব্যক্তিগত নোটবুক",
      headline1: "শেখার সাথে,",
      headline2: "শেয়ার করার গল্প।",
      bio: "প্রতিটা লেখা আসলে আমার নিজের বোঝার চেষ্টা — React, Next.js, TypeScript নিয়ে। যদি আপনিও একই জায়গায় আটকে থাকেন, হয়তো কাজে লাগবে। বাংলায় লিখি, কারণ ইংরেজি বুঝতে গিয়ে অনেক সময় আসল জিনিসটাই হারিয়ে যায়।",
      currentlyOn: "এখন কাজ করছি:",
      readArticles: "লেখাগুলো পড়ুন",
      aboutMe: "পরিচয়",
      featuredLabel: "ফিচার্ড লেখা",
      latestLabel: "সর্বশেষ লেখা",
      seeAll: "সব দেখুন",
      moreArticles: "আরও লেখা",
      allArticles: "সমস্ত লেখা",
      categoriesLabel: "বিষয়ভিত্তিক",
      seeAllCats: "সব বিষয় দেখুন",
      journalLabel: "ডেভেলপার জার্নাল",
      journalDesc:
        "ছোট ছোট চিন্তা, আবিষ্কার, আর শেখার মুহূর্ত। দীর্ঘ লেখার বাইরে যা মাথায় আসে, এখানে নোট করে রাখি।",
      seriesLabel: "শেখার সিরিজ",
      partLabel: "পর্ব",
      soonLabel: "শীঘ্রই",
      partsPublished: "পর্ব প্রকাশিত",
      newsletter: "নতুন নোট পেতে চান?",
      newsletterDesc:
        "যখনই নতুন কিছু লিখব, আপনার ইনবক্সে পৌঁছে দেব। কোনো স্প্যাম নেই, শুধু নোটবুকের পাতা।",
      emailPlaceholder: "আপনার ইমেইল ঠিকানা",
      subscribe: "সাবস্ক্রাইব",
      unsubscribeNote: "যেকোনো সময় আনসাবস্ক্রাইব করা যাবে।",
      featured: "ফিচার্ড",
      readNow: "পড়ুন",
      readingTime: "মিনিট পড়া",
    },
    // Articles
    articles: {
      pageEyebrow: "সংগ্রহশালা",
      pageTitle: "সব লেখা",
      pageDesc: "টি বিষয়ে সব লেখা।",
      filterAll: "সব",
      minRead: "মিনিট",
    },
    // Search
    search: {
      title: "সার্চ",
      placeholder: "লেখা খুঁজুন...",
      resultsText: "টি ফলাফল পাওয়া গেছে",
      emptyText: "উপরে লিখুন...",
      noResults: "কোনো ফলাফল পাওয়া যায়নি।",
    },
    // 404
    notFound: {
      title: "পাতা পাওয়া যায়নি",
      desc: "এই পাতাটি সরানো হয়েছে, অথবা এটি কখনো ছিলই না।",
      home: "হোমে ফিরুন",
      articles: "লেখা দেখুন",
    },
    // Common
    common: {
      readMore: "আরও পড়ুন",
      publishedAt: "প্রকাশিত",
      category: "বিষয়",
      tags: "ট্যাগ",
      author: "লেখক",
      series: "সিরিজ",
      part: "পর্ব",
      soon: "শীঘ্রই",
      articles: "টি লেখা",
      backToAll: "← সব লেখা",
    },
    // Learn / curriculum
    learn: {
      navLabel: "শেখা",
      eyebrow: "প্রোডাকশন লেভেল",
      title: "Next.js হ্যান্ডবুক",
      desc: "তেইশটি অধ্যায়ে production-level Next.js — architecture থেকে scale পর্যন্ত।",
      chapters: "অধ্যায়",
      chapter: "অধ্যায়",
      topics: "টপিক",
      searchTopics: "টপিক খুঁজুন...",
      noResults: "কিছু পাওয়া যায়নি।",
      soon: "শীঘ্রই",
      inThisChapter: "এই অধ্যায়ে যা আছে",
      prev: "আগের অধ্যায়",
      next: "পরের অধ্যায়",
      onThisPage: "এই পাতায়",
      comingTitle: "এই অধ্যায়টি এখনো লেখা হয়নি",
      comingDesc: "নিচের টপিকগুলো নিয়ে এই অধ্যায়টি শীঘ্রই আসছে।",
      backToIndex: "সব অধ্যায়",
      menu: "অধ্যায়সমূহ",
      progress: "প্রকাশিত",
      // Progress / journey
      yourJourney: "তোমার যাত্রা",
      complete: "সম্পন্ন",
      chaptersDone: "অধ্যায় শেষ",
      topicsDone: "টপিক পড়া",
      written: "লেখা হয়েছে",
      continueRead: "পড়া চালিয়ে যাও",
      startJourney: "যাত্রা শুরু করো",
      upNext: "এরপর",
      rank: "স্তর",
      markDone: "সম্পন্ন হিসেবে চিহ্নিত করো",
      markUndone: "সম্পন্ন চিহ্ন সরাও",
      resetProgress: "অগ্রগতি মুছে ফেলো",
      chapterMap: "অধ্যায় মানচিত্র",
      recentlyAdded: "সদ্য লেখা হয়েছে",
    },
    // Lang toggle
    lang: {
      toggle: "English",
      current: "বাংলা",
    },
    // Theme
    theme: {
      light: "আলো মোড",
      dark: "অন্ধকার মোড",
    },
  },

  en: {
    nav: {
      learn: "Learn",
      articles: "Articles",
      journal: "Journal",
      series: "Series",
      projects: "Projects",
      about: "About",
      search: "Search",
      menuOpen: "Open menu",
      menuClose: "Close menu",
      home: "Home",
    },
    footer: {
      tagline: "A Bangladeshi developer's personal notebook",
      description:
        "A personal notebook on web development, design systems, and the everyday craft of writing code — a quiet place where learning, building, and ideas are gathered with care.",
      articles: "Articles",
      journal: "Journal",
      series: "Series",
      projects: "Projects",
      about: "About",
      uses: "Uses",
      contact: "Contact",
      copyright: "Crafted with care",
      sectionContent: "Content",
      sectionMore: "More",
      sectionConnect: "Connect",
      contactUs: "Contact us",
      privacy: "Privacy policy",
      terms: "Terms & conditions",
      sitemap: "Sitemap",
      available: "Available for work",
      backToTop: "Back to top",
    },
    home: {
      eyebrow: "Personal Notebook",
      headline1: "My notes,",
      headline2: "written for you too.",
      bio: "Every post here is really me trying to understand something — React, Next.js, TypeScript. If you're stuck at the same place, it might help. I write in Bangla, because the point often gets lost in translation.",
      currentlyOn: "Currently working on:",
      readArticles: "Read Articles",
      aboutMe: "About Me",
      featuredLabel: "Featured Article",
      latestLabel: "Latest Articles",
      seeAll: "See all",
      moreArticles: "More Articles",
      allArticles: "All Articles",
      categoriesLabel: "By Topic",
      seeAllCats: "See all topics",
      journalLabel: "Developer Journal",
      journalDesc:
        "Short thoughts, discoveries, and learning moments. Quick notes from beyond the long-form articles.",
      seriesLabel: "Learning Series",
      partLabel: "Part",
      soonLabel: "Coming soon",
      partsPublished: "parts published",
      newsletter: "Want new notes?",
      newsletterDesc:
        "Whenever I write something new, I'll deliver it to your inbox. No spam, just notebook pages.",
      emailPlaceholder: "Your email address",
      subscribe: "Subscribe",
      unsubscribeNote: "Unsubscribe anytime.",
      featured: "Featured",
      readNow: "Read",
      readingTime: "min read",
    },
    articles: {
      pageEyebrow: "Archive",
      pageTitle: "All Articles",
      pageDesc: "articles on various topics.",
      filterAll: "All",
      minRead: "min",
    },
    search: {
      title: "Search",
      placeholder: "Search articles...",
      resultsText: "results found",
      emptyText: "Start typing...",
      noResults: "No results found.",
    },
    notFound: {
      title: "Page Not Found",
      desc: "This page has been removed, or never existed.",
      home: "Go Home",
      articles: "Browse Articles",
    },
    common: {
      readMore: "Read more",
      publishedAt: "Published",
      category: "Category",
      tags: "Tags",
      author: "Author",
      series: "Series",
      part: "Part",
      soon: "Coming soon",
      articles: "articles",
      backToAll: "← All articles",
    },
    learn: {
      navLabel: "Learn",
      eyebrow: "PRODUCTION LEVEL",
      title: "The Next.js Handbook",
      desc: "Production-level Next.js in twenty-three chapters — from architecture to scale.",
      chapters: "Chapters",
      chapter: "Chapter",
      topics: "topics",
      searchTopics: "Search topics...",
      noResults: "Nothing found.",
      soon: "Soon",
      inThisChapter: "In this chapter",
      prev: "Previous",
      next: "Next",
      onThisPage: "On this page",
      comingTitle: "This chapter is not written yet",
      comingDesc: "It is coming soon, and will cover the topics below.",
      backToIndex: "All chapters",
      menu: "Chapters",
      progress: "published",
      // Progress / journey
      yourJourney: "Your journey",
      complete: "complete",
      chaptersDone: "Chapters done",
      topicsDone: "Topics read",
      written: "Written",
      continueRead: "Continue reading",
      startJourney: "Start the journey",
      upNext: "Up next",
      rank: "Rank",
      markDone: "Mark as done",
      markUndone: "Mark as unread",
      resetProgress: "Reset progress",
      chapterMap: "Chapter map",
      recentlyAdded: "Recently written",
    },
    lang: {
      toggle: "বাংলা",
      current: "English",
    },
    theme: {
      light: "Light mode",
      dark: "Dark mode",
    },
  },
} as const;

export type Translations = (typeof translations)[Locale];
